...
Compatibility that isn't tested is just a promise (design §4.7). Loom ships a
...
```
1e20cf4f6d73f601b14b01a38a0e827ea4b3d873b1f401dbc7c743103e9c23557ca2
```

It pins the frozen surfaces (design §4.1):

- **Object encodings** — canonical LOM1 bytes and blake3 identities for blob,
...
```sh
loom conform          # verify this build against the golden suite
loom conform --id     # print this build's suite identity
loom conform --emit   # print this build's vectors (canonical JSON)
```
...
1. Each release's binary must pass `loom conform` against the golden suite.
2. Two releases interoperate iff their `loom conform --emit` outputs agree on
...
(`internal/gitport`) proves loom↔git round-trips reproduce byte-identical git

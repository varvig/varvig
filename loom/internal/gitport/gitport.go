// Package gitport translates between a Loom repository and a plain Git
// repository (design §2, lossless bidirectional Git export).
//
// # Losslessness
//
// Git → Loom → Git reproduces byte-identical git objects, and therefore
// identical git commit SHAs. Blobs and trees are deterministic functions of
// content, so they reconstruct exactly. Commits carry data Loom's change model
...
// git object body as an interop field on the Loom change (tag GitCommitBody).
// On export that body is re-emitted verbatim.
//
// Loom-native changes have no original git object, so their git commit is
// synthesized deterministically (UTC, author == committer). Such a change's
// Loom identity is not expected to survive a git bounce unchanged — only git
...
import (
	...
	"github.com/dividebyzero/claude-experiments/loom/internal/gitobj"
...
// Export writes the Loom change reachable from loomHead into gitDir as loose
// git objects, sets refs/heads/<branch> to the resulting commit, and points
// HEAD at it. It returns the git commit id.
func Export(r *repo.Repo, gitDir, branch string, loomHead multihash.Multihash) (gitobj.OID, error) {
...
func (e *exporter) change(o *object.Object) (gitobj.OID, error) {
	...
	// Loom-native change: synthesize a deterministic commit. Parent order
...
// Loom objects, and sets the Loom ref refs/heads/<branch>. Returns the Loom
// change id.
func Import(r *repo.Repo, gitDir, branch string) (multihash.Multihash, error) {
	...
	if err := r.Refs.CompareAndSwap(name, cur, loomID, "git-import", "import from git"); err != nil {
		return nil, err
	}
	return loomID, nil
}
...
// normalizeIdent turns a Loom author string into a git identity "Name <email>".

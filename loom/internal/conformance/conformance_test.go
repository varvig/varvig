...
// deliberately (LOOM_WRITE_GOLDEN=1) when the frozen format legitimately gains
// a new vector — never as part of a normal test run.
func TestWriteGolden(t *testing.T) {
	if os.Getenv("LOOM_WRITE_GOLDEN") == "" {
		t.Skip("set LOOM_WRITE_GOLDEN=1 to regenerate vectors.json")
	}

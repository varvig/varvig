...
func TestWriteTreeCheckoutRoundTrip(t *testing.T) {
	...
	// A .varvig directory must be ignored.
	if err := os.MkdirAll(filepath.Join(src, ".varvig", "objects"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	// .varvig must not have been captured.
	if _, err := os.Stat(filepath.Join(dst, ".varvig")); !os.IsNotExist(err) {
		t.Fatal(".varvig leaked into the tree")
	}
